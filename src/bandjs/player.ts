 ...
 */
import type Conductor from './band-js' // Use type import
import type Instrument from '../song-player/instrument' // Add Instrument type import
type IGainNode<T> = any
type IAudioScheduledSourceNode<T> = any
type IAudioContext = any
...
export default class Player {
  ...
  private bufferTimeout: any | undefined
  ...
  constructor(conductor: Conductor) {
    ...
    this.getCurrentPlayTime = () => this.currentPlayTime
    this.getTotalPlayTime = () => this.totalPlayTime
    this.getTimeOffset = () =>
  ...
  private reset(resetDuration = false): void {
    ...
    while (++index < numOfInstruments) {
      ...
      if (resetDuration) {
        ...
        instrument.bufferPosition = this.findBufferPositionForTime(
          instrument,
          this.totalPlayTime
        )
      }
  ...
  private findBufferPositionForTime(
    ...
    while (++index < this.conductor.instruments.length) {
      ...
      while (++index2 < bufferCount) {
        ...
        if (Array.isArray(pitch)) {
          pitch.slice(1).forEach((additionalPitch) => {
            const gain2 = this.conductor.audioContext.createGain()
            gain2.connect(this.conductor.masterVolume)
            gain2.gain.value = currentVolumeLevel

            let resolvedPitchValue2

            if (typeof additionalPitch === 'string') {
              resolvedPitchValue2 =
                this.conductor.pitches[additionalPitch.trim()] ||
                Number.parseFloat(additionalPitch)
            } else {
              resolvedPitchValue2 = additionalPitch // It's already a number
            }

            const audioNode2 = instrument.instrument.createNote(
              gain2,
              resolvedPitchValue2
            )
    ...
    const playNotesInternal = (notesToPlay: PlayerBufferedNote[]) => {
      ...
      while (++index < notesToPlay.length) {
        ...
        if (!note.tie) {
          ...
          if (note.node.frequency) {
            const f = note.node.frequency.value

            if (duration < 2) {
              /*note.node.frequency.linearRampToValueAtTime(
                f + 30,
                note.startTime + (note.stopTime - note.startTime) / 2
              )

              note.node.frequency.setValueAtTime(f, note.stopTime + 0.01)*/
            }
            /*note.node.frequency.linearRampToValueAtTime(
              f + 130,
              note.startTime + (note.stopTime - note.startTime) / 2
            )*/
            //note.node.frequency.linearRampToValueAtTime(f, note.stopTime)
        ...
        const noteOverallDuration = noteAbsoluteStopTime - noteAbsoluteStartTime
